...
static uint8 TrainBLEPeripheral_TaskID;   // Task ID for internal task/event processing
...
extern CONST uint8 advertData[17];

/*********************************************************************
 * LOCAL FUNCTIONS
 */
static void TrainBLEPeripheral_ProcessOSALMsg( osal_event_hdr_t *pMsg );
static void TrainBLEPeripheral_ProcessGATTMsg( gattMsgEvent_t *pMsg );
...
// GAP Role Callbacks
static gapRolesCBs_t TrainBLEPeripheral_PeripheralCBs =
{
...
static gapBondCBs_t TrainBLEPeripheral_BondMgrCBs =
{
...
static trainProfileCBs_t TrainBLEPeripheral_TrainProfileCBs =
{
 ...
 * @return  none
 */
void TrainBLEPeripheral_Init( uint8 task_id )
{
  ...
  TrainBLEPeripheral_TaskID = task_id;
  ...
  VOID TrainProfile_RegisterAppCBs( &TrainBLEPeripheral_TrainProfileCBs );
  ...
  osal_set_event( TrainBLEPeripheral_TaskID, SBP_START_DEVICE_EVT );
 ...
 */
uint16 TrainBLEPeripheral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & SYS_EVENT_MSG )
  {
    ...
    if ( (pMsg = osal_msg_receive( TrainBLEPeripheral_TaskID )) != NULL )
    {
      TrainBLEPeripheral_ProcessOSALMsg( (osal_event_hdr_t *)pMsg );
  ...
  {
    // Start the Device
    VOID GAPRole_StartDevice( &TrainBLEPeripheral_PeripheralCBs );

    // Start Bond Manager
    VOID GAPBondMgr_Register( &TrainBLEPeripheral_BondMgrCBs );

    // Set timer for first periodic event
    osal_start_timerEx( TrainBLEPeripheral_TaskID, SBP_PERIODIC_EVT, SBP_PERIODIC_EVT_PERIOD );
...
static void TrainBLEPeripheral_ProcessOSALMsg( osal_event_hdr_t *pMsg )
{
  ...
  {     
    case GATT_MSG_EVENT:
      ...
      TrainBLEPeripheral_ProcessGATTMsg( (gattMsgEvent_t *)pMsg );
      break;
